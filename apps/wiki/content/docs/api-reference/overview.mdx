...
| ------ | ------------------------------------- | -------------------------------------------------------------------------------------------- |
| GET    | `/contacts`                           | List contacts. Supports `search` (by email substring), `subscribed`, `tags` (comma-separated tag IDs, matches ANY), `limit`, `cursor`. Each contact includes its `tags: {id, name}[]`. |
...
| Method | Path                          | Description                                                                                  |
| ------ | ----------------------------- | -------------------------------------------------------------------------------------------- |
| GET    | `/tags`                       | List all tags for the project, with cached `memberCount` (no pagination — small list).       |
| GET    | `/tags/:id`                   | Get a single tag.                                                                            |
| POST   | `/tags`                       | Create a tag. Body: `{ name }`. Names are unique per project, case-insensitively.            |
| PATCH  | `/tags/:id`                   | Rename a tag. Body: `{ name }`. Safe to do at any time — campaigns, workflows, and segment/filter conditions bind by tag ID, never by name. |
| DELETE | `/tags/:id`                   | Delete a tag. Returns 409, enumerating references, if it's used by an active campaign, an enabled workflow trigger/step, or a segment/filter condition. |
| GET    | `/tags/:id/contacts`          | Page-based list of contacts with this tag. `page`, `pageSize` (max 100).                     |
| POST   | `/tags/contacts/:contactId`   | Apply one or more tags to a contact. Body: `{ tagIds }`. Already-applied tags are a silent no-op. |
| DELETE | `/tags/contacts/:contactId`   | Remove one or more tags from a contact. Body: `{ tagIds }`.                                  |
| POST   | `/tags/bulk-apply`            | Bulk add/remove tags across many contacts. Queued — returns a `jobId`. Body is `{ mode: "ids", contactIds, tagIds, action }` or `{ mode: "query", filter: { search?, subscribed?, tagIds? }, excludeIds?, tagIds, action }`, where `action` is `"add"` or `"remove"`. |
| GET    | `/tags/bulk-apply/:jobId`     | Poll the status of a bulk tag job. `result` reports `{ action, totalRequested, successCount, unchangedCount, failureCount }`. |
...
| **Steps** |                                                |                                                                                          |
| POST   | `/workflows/:id/steps`                            | Add a step (`SEND_EMAIL`, `DELAY`, `WAIT_FOR_EVENT`, `CONDITION`, `WEBHOOK`, `UPDATE_CONTACT`, `ADD_TAG`, `REMOVE_TAG`, `EXIT`). |
