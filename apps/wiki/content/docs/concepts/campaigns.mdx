...
<TypeTable
  type={{
    ...
    audienceType: { type: 'string', required: true, description: 'Who this campaign is sent to. One of `ALL`, `SEGMENT`, `FILTERED`, or `TAG` — see below.' },
    ...
    tagIds: { type: 'string[]', description: 'Required when `audienceType = TAG`. Contacts with ANY of these tags are included.' },
    excludeTagIds: { type: 'string[]', description: 'Optional when `audienceType = TAG`. Contacts with ANY of these tags are subtracted from the include set.' },
...
Campaigns support four audience types:
...
- **`TAG`** — contacts carrying any of `tagIds`, minus contacts carrying any of `excludeTagIds`. A one-click picker for [tag](/concepts/tags)-based audiences ("customers except launch-optouts") without building a filter tree. Matching against *all* of several tags isn't a picker mode — use `FILTERED` with `hasTag` conditions combined with `AND` for that.
...
<Cards>
  ...
  <Card title="Tags" href="/concepts/tags">
    Build audiences for `audienceType: TAG` campaigns.
  </Card>
  <Card title="Sequences" href="/concepts/sequences">
    The growing alternative to a one-off send — an ordered series each contact walks at their own pace.
  </Card>
