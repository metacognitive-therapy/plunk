...
A contact can be identified without being email-reachable (a lead), email-reachable without being identified (a typical contact created via `/v1/track` or `POST /contacts`), both, or neither. The dashboard's contact list marks leads distinctly, and the headline contact count is split into mailable contacts and leads so the number you see matches the number a campaign will actually reach.

<Callout title="Adding an email to a lead" type="info">
  This version of `/v1/identify` only covers the simplest case — creating a lead from an external id.
  Resolving an external id against an existing email-bearing contact (or adding an email to an
  existing lead) is a separate, more involved operation and isn't covered here yet.
</Callout>
