...
"Delete" — from the dashboard, `DELETE /contacts/:id`, `DELETE /contacts/external/:externalId`, their bulk equivalents, or the MCP `plunk_delete_contact` tool — **anonymizes** the contact rather than removing its row. The contact's `email` is nulled, its `data` attributes are cleared, and the payload is stripped from every event it fired, then `deletedAt` is set. There is no option to hard-delete instead: every one of those entry points resolves to the same anonymize operation, with no flag to bypass it.
