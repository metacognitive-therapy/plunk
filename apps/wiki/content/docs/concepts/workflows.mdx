...
| ------------ | ------------------------------------------------------------------------------------------------------ |
| `EVENT`      | Any time a matching event is tracked on a contact (custom event via `/v1/track`, system events such as `email.received` or `contact.subscribed`, tag events, or segment entry/exit events). |
...
An `EVENT` trigger can additionally be scoped to one specific [tag](/concepts/tags): `triggerConfig: { eventName: "tag.added" | "tag.removed", tagId: "<id>" }`. This fires only when *that* tag is applied or removed, rather than any tag change on the contact. Because it binds by tag ID, renaming the tag later never breaks the trigger.
...
| `UPDATE_CONTACT`  | Patches contact data — useful for progressing custom fields as a contact moves through the journey (`{ stage: "activated" }`). | `data` object                                                    |
| `ADD_TAG`         | Applies a [tag](/concepts/tags) to the contact flowing through the workflow. Fires `tag.added`, so it can chain into another workflow. | `tagId`                                                           |
| `REMOVE_TAG`      | Removes a tag from the contact. Fires `tag.removed`.                                                   | `tagId`                                                           |
...
- **Webhook fan-out** — `EVENT` trigger on `purchase.completed`, `WEBHOOK` step to your CRM, `ADD_TAG` to label the contact `customer`, `SEND_EMAIL` with the receipt.
- **Tag-driven sequence** — `EVENT` trigger scoped to a tag (`tag.added` on the "trial" tag), send a series of onboarding emails, then `ADD_TAG` "nurtured" so a downstream workflow can pick up from there.
...
<Cards>
  ...
  <Card title="Tags" href="/concepts/tags">
    Trigger workflows from tag added / removed events, and maintain tags with `ADD_TAG` / `REMOVE_TAG` steps.
  </Card>
</Cards>
