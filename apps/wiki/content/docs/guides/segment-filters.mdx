...
For `event.<eventName>`, the time-bounded operators (`triggeredWithin`, `triggeredOlderThan`, `notTriggeredWithin`) evaluate against **when the event occurred** (`occurredAt`, settable via `POST /v1/track` — see [Track an event](/api-reference/public-api/trackEvent)), not when Plunk received it. This keeps a delayed or retried tracking call from landing in the wrong recency window just because it arrived late. `email.<activity>` operators are unaffected — they always read the corresponding email timestamp (`openedAt`, `clickedAt`, etc).
